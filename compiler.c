...
#define EMIT_LONG(op, arg) \
    do { \
        if (arg > UINT8_MAX) { \
            emitByte(op##_LONG); \
            emitLong(arg); \
        } \
        else emitBytes(op, arg); \
    } while (false)
...
static void initCompiler(Compiler* compiler, FunctionType type) {
    ...
    compiler->locals = GROW_ARRAY(Local, NULL, 0, UINT8_COUNT);
...
static void continueStatement() {
    if (currentLoopStart <= 0) {
        error("Continue statement must be inside of a loop.");
    }
    consume(TOKEN_SEMICOLON, "Expect ';' after continue.");
    for (int i = current->localCount - 1; i >= 0; i--) {
        if (current->locals[i].depth > currentLoopScopeDepth) emitByte(OP_POP);
    }
...
static void forStatement() {
    ...
    int previousLoopStart = currentLoopStart;
    startLoop();
    int exitJump = -1;
...
static void switchStatement() {
    ...
    while (!check(TOKEN_DEFAULT) && !check(TOKEN_EOF)) {
        emitByte(OP_COPY);
        consume(TOKEN_CASE, "Expect 'case' to mark switch case.");
        expression();
        ...
        if (endJumpCapacity < endJumpCount + 1) {
            int oldCapacity = endJumpCapacity;
            endJumpCapacity = GROW_CAPACITY(oldCapacity);
            endJumps = GROW_ARRAY(int, endJumps, oldCapacity, endJumpCapacity);
        }
    ...
    if (!match(TOKEN_DEFAULT)) error("No default case for switch statement.");
...
static void namedVariable(Token name, bool canAssign) {
    ...
    bool expectExpression = canAssign && match(TOKEN_EQUAL);
    if (arg != -1) {
        if (expectExpression) {
            expression();
            EMIT_LONG(OP_SET_LOCAL, arg);
        }
        else {
            EMIT_LONG(OP_GET_LOCAL, arg);
        }
    }
    else {
        ...
        if (expectExpression) {
            expression();
            EMIT_LONG(OP_SET_GLOBAL, arg);
        }
        else {
            EMIT_LONG(OP_GET_GLOBAL, arg);
        }
    }
}
...
static int identifierConstant(Token* name) {
    return addConstant(currentChunk(), OBJ_VAL(copyString(name->start, name->length, true)));
}
